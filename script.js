document.getElementById("assetTypeInput").addEventListener("input", function() {
  ...
  let appServiceRow = document.getElementById("appServiceRow");
  ...
  if (assetType === "management group") {
  ...
  } else if (assetType === "resource group"){
    appServiceRow.style.display = "block";
    purposeRow.style.display = "block";
    numberRow.style.display = "block";
  } else {
    ...
    appServiceRow.style.display = "none";
  }
...
function generateResourceGroup() {
  let appServiceInput = document.getElementById("appServiceInput").value;
  let checkAppService = document.getElementById("checkAppService");
  let purposeInput = document.getElementById("purposeInput").value;
  let checkPurpose = document.getElementById("checkPurpose");
  let numberInput = document.getElementById("numberInput").value;
  let appServiceString
  let purposeString
  let numberString

  if (checkAppService.checked) {
    appServiceString = abbreviateWord(appServiceInput);
  } else {
    appServiceString = appServiceInput
  }
  ...
  if (appServiceString && purposeString && numberString) {
    let subscription = "rg" + "-" + appServiceString + "-" + purposeString + "-" + numberString
    let modalBody = document.querySelector(".name-results");
    ...
    const toastElement  = document.getElementById('toastCopyErrorResourceGroup'); 
    let toast = new bootstrap.Toast(toastElement);
    toast.show()
  }
}
...
function clearForm() {
  ...
  document.getElementById("appServiceInput").value = "";
